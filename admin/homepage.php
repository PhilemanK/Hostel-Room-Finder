...
<body>
  <section class="background">
    ...
    <!--Program to fetch no of hr-->
    <?php
    $conn = mysqli_connect("localhost", "root", "", "hr");
    $db = mysqli_select_db($conn, "hr");
    $hr_query = "SELECT * FROM hr_table";
    if ($hr_result = mysqli_query($conn, $hr_query)) {
      // Return the number of rows in result set
      $hr_rowcount = mysqli_num_rows($hr_result);
    }
    ?>

    <!--Program to fetch no of hrAdmin-->
    <?php
    $conn = mysqli_connect("localhost", "root", "", "hr");
    $db = mysqli_select_db($conn, "hr");
    $hr_admin_query = "SELECT * FROM hr_admin";
    if ($hr_admin_result = mysqli_query($conn, $hr_admin_query)) {
      // Return the number of rows in result set
      $hr_admin_rowcount = mysqli_num_rows($hr_admin_result);
    }
    ?>



    <!--DISPLAYING TOTAL NO OF USER< HR ADMIN AND HR-->
    <div class="container">
      <div class="row g-2">
        <div class="col-lg-4">
          <div class="p-5 border text-center fs-2" style="border-radius: 15px; border:#800000 solid; background-color:#FFFFFF; color:#800000"><b>Total Users</b><br>
            <p id="user_id" onload="user()"><?php echo "$rowcount"; ?></p>
          </div>
        </div>
      </div>
      <script>
        ...
        incEltNbr("user_id"); /*Call this funtion with the ID-name for that element to increase the number within*/
      </script>
      ...
      <div class="row g-2 justify-content-center">
        <div class="col-lg-4">
          <div class="p-5 border text-center fs-2" style="border-radius: 15px;  border:#800000 solid; background-color:#FFFFFF; color:#800000"><b>Total HR Admins</b><br>
            <p id="hr_admins_id" onload="hr_admin()"><?php echo "$hr_admin_rowcount"; ?></p>
          </div>
        </div>
      </div>
      <script>
        ...
        /*Function called on button click*/
        function hr_admin() {
          incEltNbr("hr_admins_id");
        }
        incEltNbr("hr_admins_id"); /*Call this funtion with the ID-name for that element to increase the number within*/
      </script>
      ...
      <div class="row g-2 justify-content-end">
        <div class="col-lg-4">
          <div class="p-5 border text-center fs-2" style="border-radius: 15px;  border:#800000 solid; background-color:#FFFFFF; color:#800000"><b>Total H/R's</b><br>
            <p id="hr_id" onload="hr()"><?php echo "$hr_rowcount"; ?></p>
          </div>
        </div>
        <script>
          var speed = 100;
          ...
          incEltNbr("hr_id"); /*Call this function with the ID-name for that element to increase the number within*/
        </script>
      </div>
    </div>
    <!--CONTAINER END-->
